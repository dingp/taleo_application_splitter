...
- `python3` (Python 3.9+ recommended)
...
The scripts use only the Python standard library, including modules such as:
...
- `split_applications.py` requires all four Poppler tools listed above.
- `extract_applicants_to_xlsx.py` requires `pdftotext`.
- `update_email_columns.py` uses only Python standard library modules and reads/writes `.xlsx` files directly as ZIP/XML content.

On this machine, the PDF command-line tools are available from Poppler.
